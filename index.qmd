...
[L. Kennedy]{style="text-align: right; font-size: 75%;"}
:::
...
```{r}
pbmc_umap <- readRDS("data/pbmc_umap.rds")
pbmc_pca <- readRDS("data/pbmc_pca_50.rds")
langevitour::langevitour(pbmc_pca)
...
```{r}
df_exe <- read.csv("data/df_exe_better.rds")
distance_df_small_edges <- read.csv("data/distance_df_small_edges_better.rds")
langevitour::langevitour(df_exe[1:(length(df_exe)-1)], lineFrom = distance_df_small_edges$from, lineTo = distance_df_small_edges$to, group = df_exe$type, levelColors = c("#807dba" , "#000000"))
```
...
```{r}
readRDS("data/better_umap_plot.rds")
```
